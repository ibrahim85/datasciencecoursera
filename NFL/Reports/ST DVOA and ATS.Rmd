...
NFLCorr$Year=factor(NFLCorr$Year)
```
...
```{r, echo=FALSE}
qplot(Week.3.4.ST.DVOA,ATS..,color=Year,data=NFLCorr)
...
```{r, echo=FALSE}
with(NFLCorr, plot(Week.3.4.ST.DVOA,ATS..))
...
As an afterthought, let's look at a hierarchical clustering of our plot. Since we'll be using Euclidean distance in this investigation, we will first norm our data so as to not give undue preference to ST DVOA (which has much greater variance).

```{r, echo=FALSE}
normATS <- (NFLCorr$ATS..-mean(NFLCorr$ATS..))/sd(NFLCorr$ATS..)
normST <- (NFLCorr$Week.3.4.ST.DVOA-mean(NFLCorr$Week.3.4.ST.DVOA))/sd(NFLCorr$Week.3.4.ST.DVOA)
normedATSandST <- cbind(normATS,normST)
plot(normedATSandST)
```

Now, we will cluster our normed data.

```{r, echo=FALSE}
distNormATSandST <- dist(normedATSandST)
normCluster <- hclust(distNormATSandST)
plot(normCluster)
```

There appear to be 4 roughly even clusters, so we will make our cuts there.

```{r, echo=FALSE}
clusterNFL <- cbind(NFLCorr$ATS..,NFLCorr$Week.3.4.ST.DVOA,cutree(normCluster,4))
qplot(clusterNFL[,2],clusterNFL[,1],color=clusterNFL[,3])
```

This actually isolates the same 18 teams (average 9 per season) selected to maximize ATS in the earlier analysis. However, it selects 10 teams (average 5 per season) on the lower end.
